...
public class Driving2 : MonoBehaviour
{
	public float accel;
	...
	float rb_speed_forward;
	float rb_speed_right;
	...
	void FixedUpdate()
	{
		...
		rb_speed_forward = rb_speed_local[0]; // splits local space vector into forward component and right component
		rb_speed_right = rb_speed_local[1];
		...
		rb_speed_right *= 0.95f; // always lowers right/left speed of car (will not effect drift dont worry)
		if (math.abs(rb_speed_right) < 0) rb_speed_right = 0;
        #endregion
        ...
        if (steering.IsPressed())
        {
            ...
            {
                rb_direction -= steering.ReadValue<float>() * steering_speed * Time.fixedDeltaTime;
            }
            else if (drivingState == DrivingState.barckward)
            {
                rb_direction += steering.ReadValue<float>() * steering_speed * Time.fixedDeltaTime;
